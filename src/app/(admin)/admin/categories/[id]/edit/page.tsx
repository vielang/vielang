...
export default function EditCategoryPage() {
  ...
  const [currentCategory, setCurrentCategory] = React.useState<ContentCategory | null>(null)
  ...
  }, [admin, authLoading, categoryId])

  async function loadCategory() {
    try {
      setIsLoadingCategory(true)
      const category = await adminCategoryApi.getById(Number(categoryId))
      ...
      form.reset({
        ...
        isNav: toBoolean(category.isNav),
  ...
  async function loadCategories() {
    try {
      ...
      // Only show root categories as parent options, and exclude the current category
      setCategories(
        result.filter(
          (cat) => cat.parentId === 0 && cat.id !== Number(categoryId)
        ) || []
      )
    } catch (error) {
  ...
  async function onSubmit(data: CategoryFormValues) {
    ...
    try {
      const categoryData = {
        ...
        sort: data.sort || 0,
        status: data.status, // Boolean, not number
        isNav: data.isNav, // Boolean, not number
      }

      // Update category with all data including status and isNav
      await adminCategoryApi.update(Number(categoryId), categoryData)
  ...
  if (authLoading || isLoadingCategory) {
    return (
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Update category information for {currentCategory?.name}
        </p>
      </div>
